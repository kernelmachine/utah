...
/// ```
    fn sumdf(&'a mut self,
             axis: UtahAxis)
             -> Sum<'a, DataFrameIterator<'a, T, S>, T, S> {
...
/// ```
    fn mean(&'a mut self,
            axis: UtahAxis)
            -> Mean<'a, DataFrameIterator<'a, T, S>, T, S> {
...
/// ```
    fn maxdf(&'a mut self,
             axis: UtahAxis)
             -> Max<'a, DataFrameIterator<'a, T, S>, T, S> {
...
/// ```
    fn min(&'a mut self,
           axis: UtahAxis)
           -> Min<'a, DataFrameIterator<'a, T, S>, T, S> {
...
/// ```
    fn stdev(&'a self,
             axis: UtahAxis)
             -> Stdev<'a, DataFrameIterator<'a, T, S>, T, S> {
        let columns = self.columns.clone();
        let index = self.index.clone();
        match axis {
...
//     /// df.is_ok();
//     /// ```
//     fn columns<U: Clone>(mut self, columns: &'a [U]) -> Result<DataFrame<f64, String>>
//         where String: From<U>
//     {
//         if columns.len() != self.data.shape()[1] {
//             return Err(ErrorKind::ColumnShapeMismatch.into());
...
//     /// let df = DataFrame::new(a).index(&[1, 2]).columns(&["a", "b"]).unwrap();
//     /// ```
//     fn index<U: Clone>(mut self, index: &'a [U]) -> Result<DataFrame<f64, String>>
//         where String: From<U>
//     {
//         if index.len() != self.data.shape()[0] {
//             return Err(ErrorKind::RowShapeMismatch.into());
